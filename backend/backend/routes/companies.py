...
# @router.get("/liked", response_model=CompanyBatchOutput)
def fetch_companies_with_liked(
    ...
    liked_companies = {
        association.company_id for association in liked_associations}

    companies = (
        db.query(database.Company).filter(
            database.Company.id.in_(company_ids)).all()
    )

    results = [(company, company.id in liked_companies)
...
def get_companies(
    ...
    companies = fetch_companies_with_liked(
        db, [company.id for company in results])
